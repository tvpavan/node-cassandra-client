...
var ALREADY_CONNECTING_MESSAGE = 'Already connecting';
...
var Row = module.exports.Row = function(row, decoder) {
  ...
  for (var i = 0; i < row.columns.length; i++) {
    if (row.columns[i].value !== null && row.columns[i].name != 'KEY') {
...
Connection.prototype.connect = function(callback) {
  var self = this,
      ...
      callbackWrapper = function(err) {
        if (!cbAlreadyReturned) {
          ...
          if (err && err.message === ALREADY_CONNECTING_MESSAGE) {
            // do nothing
          } else {
  ...
  if (self._connecting) {
    callbackWrapper(new Error(ALREADY_CONNECTING_MESSAGE));
    return;
  } else {
...
Connection.prototype.close = function(callback) {
  ...
  if (!self.con) {
    // Don't call end if con is not set (e.g timeout while establishing a
...
ConnectionInPool.prototype.connect = function(callback) {
  ...
  if (this._connecting) {
    callback(new Error(ALREADY_CONNECTING_MESSAGE));
    return;
  }
  if (this._id !== undefined && this.con && this.client) {
    this.emit('log', 'debug', 'initiating proactive transport close', {});
    // ghetto close.
    var _con = self.con;
  ...
  this.on('connected', this.clearOnConnectedQueue);
...
ConnectionInPool.prototype.onConnected = function(callback) {
...
ConnectionInPool.prototype.clearOnConnectedQueue = function() {
  var self = this;
  if (this._onConnected === undefined) {
    return;
  }
  this._onConnected.forEach(function(callback) {
    callback();
  });
  self._onConnected = undefined;
};

ConnectionInPool.prototype._isHealthy = function() {
  return this.unhealthyAt === 0;
};

ConnectionInPool.prototype.states = {
  CONNECTED: 'CONNECTED',
...
/**
 * a 'stale' node is a node that has been bad for some period of time. After that
 * period, it is safe to retry the connection.
 */
ConnectionInPool.prototype.getState = function() {
  if (this._isHealthy()) {
    if (this.connected) {
    ...
    if (new Date().getTime() - this.unhealthyAt > this._options.staleThreshold) {
      return this.states.STALE;
    } else {
      return this.states.UNHEALTHY;
    }
  }
};
...
PooledConnection.prototype._getNextCon = function(unwrappedCallback) {
  var self = this,
      calledBack = false,
      timeoutTime = self.timeout * self.connections.length, // preserves legacy behavior regarding timeouts.
      rand = Math.floor(Math.random() * 1000), // starting index should be random.
      unhealthyCount = 0,
      i, index,
      callback = function(err, conn) {
        ...
        calledBack = true;
        unwrappedCallback(err, conn);
      };


  function ensureConnection(conn) {
    switch (conn.getState()) {
      case conn.states.CONNECTED:
        callback(null, conn);
        break;
      case conn.states.CONNECTING:
        conn.onConnected(function() {
          callback(null, conn);
        });
        break;
      case conn.states.STALE:
        // unhealthy and stale, so let reset the node (appears as if unconnected).
        self.emit('log', 'debug', 'connection ' + conn.connectionInfo.host + ':' + conn.connectionInfo.port + ' is stale and not healthy. marking it as disconnected and resetting the state', {
          'connectionInfo': conn.connectionInfo
        });
        conn.setDisconnected();
        conn.unhealthyAt = 0;
        // THERE IS NO BREAK HERE. INTENTIONALLY. after we reset state, we should reconnect
        /* falls through */
      case conn.states.DISCONNECTED:
        ...
        break;
      case conn.states.UNHEALTHY:
        self.emit('log', 'debug', 'connection ' + conn.connectionInfo.host + ':' + conn.connectionInfo.port + ' is unhealthy but not stale. passing over it.');
        unhealthyCount++;
        break;
      default:
        self.emit('log', 'debug', 'encountered an unknown state of: ' + conn.getState() + ' for connection ' + conn.connectionInfo.host + ':' + conn.connectionInfo.port);
        break;
    }
  }

  setTimeout(callback.bind(null, new Error('Timed out waiting for connection after ' + timeoutTime + 'ms.')),
                          timeoutTime);
  ...
  for (i = 0; i < self.connections.length; i++) {
    ...
    index = (i + rand) % self.connections.length;
    ensureConnection(self.connections[index]);
  }

  if (unhealthyCount === self.connections.length) {
    callback(new Error('All connections are unhealthy.'));
  }
};
