  ...
  function ensureConnection(conn) {
    switch (conn.getState()) {
      ...
      case conn.states.UNHEALTHY:
        self.emit('log', 'debug', 'connection ' + conn.host + ':' + conn.port + ' is unhealthy but not stale. passing over it.');
        unhealthyCount++;
        break;
      default:
        self.emit('log', 'debug', 'encountered an unknown state of: ' + conn.getState() + ' for connection ' + conn.host + ':' + conn.port);
        break;
    }
  }
